...
class Transformer(nn.Module):
    ...
    def __init__(self, n_features, dim, num_heads=5, num_layers=5):
        ...
        layer = nn.TransformerEncoderLayer(d_model=self.embed_dim, nhead=num_heads)
        self.transformer = nn.TransformerEncoder(layer, num_layers=num_layers)
        self.global_token = nn.Parameter(torch.randn(1, 1, self.embed_dim))
    ...
    def forward(self, x):
        ...
        # 添加全局token
        global_tokens = self.global_token.expand(1, x.size(1), -1)
        x = torch.cat([global_tokens, x], dim=0)
        
        x = self.transformer(x)
        return x[0]  # 只返回全局token
